(function (global, factory) {
	...
	(global.Kano = global.Kano || {}, global.Kano.apiClient = factory());
	...
	(function(){
				...
				function each(val, key){
					var ctx = this.ctx, graph = ctx.graph, msg = this.msg, soul = msg.get, node = msg.put, at = (msg.gun._);
				...
				Gun.on.get = function(msg, gun){
					var root = gun._, soul = msg.get[_soul], node = root.graph[soul], has = msg.get[_has];
	...
	USE(function(module){
			...
			function output(msg){
				var get, at = this.as, back = at.back, root = at.root;
				if(!msg.gun){ msg.gun = at.gun; }
				this.to.next(msg);
				if(get = msg.get){
					/*if(u !== at.put){
			...
			function map(data, key){ // Map over only the changes on every update.
				var cat = this.cat, next = cat.next || empty, via = this.at, chain, at;
