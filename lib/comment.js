...
const ENDPOINTS = {
    ...
    deleteCommentByAdmin(cid) {
        return {
            path: `/admin/comments/${cid}`,
            method: 'DELETE',
            responseType: 'text',
        };
    },
...
export class CommentClient extends Client {
    ...
    getListByIds(ids) {
        ...
        return this.fetch(endpoint)
            .then(res => res.data.comments.map(comment => expandComment(
                comment,
                res.data.avatarBaseUrl || res.data.aBurl,
            )))
    ...
    deleteCommentByAdmin(id) {
        const endpoint = this.getEndpoint('deleteCommentByAdmin', id);
        return this.fetch(endpoint)
            .then(data => this.afterDataProcessed(endpoint, data));
    }
    flagComment(id) {
