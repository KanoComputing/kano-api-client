...
export default function (settings) {
  ...
  function getter(query, params, sync) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const queryAsArray = queryRun.split('.*')[0].split('.');

      function seeDataContext(idx) {
        async function seeData(data, resolving) {
          ...
          if (sync && data === undefined) {
            ...
            } else if (queryAsArray[0] === "shares" && queryAsArray.length === 1) {
              const shares = gun.get('shares');
              return await getDataFromServer(`shares`).then(serverRes => {
            ...
            } else if (idx >= 2 && queryAsArray[idx-2] === "shares") {
              const share = gun.get('shares').get(queryAsArray[idx-1])
              return await getDataFromServer(`shares?ids=${queryAsArray[idx-1]}`).then(serverRes => {
        ...
        return seeData;
      } // TODO use "gun load"  if ".*"


      queryAsArray.reduce((db, val, idx) => db.get(val).once(seeDataContext(idx)), gun).once(data => {
  ...
  function setter(query, valueToSet) {
    ...
    }).then(() => getter(theQuery)).then(data => {
      newValue = data;
    }).then(() => {
    ...
    if (user && user.renew < Date.now() && user._accessToken) {
      onIdle(1000, 10).then(() => getDataFromServer('accounts/auth/refresh').then(res => {
        ...
        if (res.data && res.data.token) {
          ...
          ls.setItem('user', JSON.stringify(Object.assign(lUser, {
            _accessToken: token,
            renew
          })));
        } else {
  ...
  function makeLocalToken(username, password) {
    ...
    return sha256(username + password).then(localhash => crypto.subtle.importKey('raw', localhash, {
      name: 'AES-CBC'
    }, true, ['encrypt', 'decrypt'])).then(key => sha256(username).then(userSHA => {
      ...
      if (data) {
        ...
        ).then(decrypted => // TODO put ES-CBC
        // as no initial Factor I need to chop off the first 8 characters
        ls.setItem('user', ab2str(decrypted).slice(8))).then(() => {}).catch(err => {
          console.error(err);
        });
      }

      return key;
    }).then(theKey => // if encrypted data decrypt it
    crypto.subtle.exportKey('jwk', theKey)).then(keydata => // returns the exported key data
  ...
  const API = {
          ...
          if (ls.getItem(userHash)) {
            ...
            return poster(argUser, 'accounts').then(res => {
              ...
              if (res.data && res.data.token) {
                ...
                if (user.username) {
                  return makeLocalToken(user.username, args.params.user.password).then(localToken => {
                    ls.setItem('user', JSON.stringify(Object.assign(ls.user || {}, {
                      renew,
                      userHash,
                      _accessToken: token,
    ...
    read: args => new Promise(resolve => {
      resolve(API._read(Object.assign({
        sync: true
      }, args)));
    }),
    _read: async args => {
    ...
    login: args => {
      ...
      return API.read({
        ...
        if ((await user.username) === undefined) {
            ...
            if (!ls.getItem('user')) {
              return poster(args.params.user, 'accounts/auth').then(res => {
                ...
                ls.setItem('user', JSON.stringify({
                  ...
                  username: args.params.user.username,
                  _localToken: localToken,
                  // to encrypt with when logged out
                  _accessToken: token,
                  // to access server
                  renew
                }));
          ...
          if (settings.log) {
            console.log('you are (and were) logged in :)');
          }
        } else if ((await user.username) !== args.params.username) {
          return API.logout().then(() => API.login(args));
        }

        return API.read(Object.assign({
          sync: true
        }, args));
      });
    },
